...
class ExceptionInformation:
    type: str = 'root'
    stacktrace: List[str] = field(default_factory=list)
    child: Union['ExceptionInformation', 'ExceptionEndInformation', None] = \
...
def set_hierarchy_exception(exceptions_text: List[str], parent_exception: ExceptionInformation,
                            child_exception: bool = False) -> None:
    """
    :param exceptions_text:
    ...
    if child_exception:
        exception_info = ExceptionInformation(type=raw_exception_info[-1], stacktrace=raw_exception_info[:],
                                              relation=child_relation)
    else:
        exception_info = ExceptionInformation(type=raw_exception_info[0], stacktrace=raw_exception_info[:],
                                              relation=child_relation)
