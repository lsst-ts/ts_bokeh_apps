...
from lsst_ts.bokeh.apps.examples.bokeh_framework_efd.efd_example_interaction import \
...
# Auxiliar classes for plot creation
class Plot:
    _WIDTH = 400
    _HEIGHT = 400
    _ALIGN = "center"
    ...
    def add_span(self, start):
        """
        Add a span into the figure plot #Maybe can be improved being able to
        set properties of the span
        :param start:
        :return:
        """
        obs_start = Span(
...
class EfdExampleLayout(Layout):
    ...
    def __init__(self):
        ...
        # In CustomWidgets class, the framework offer some builtin widgets that
    ...
    def define(self) -> LayoutDOM:
        ...
        # Get error message viewer from the custom widgets
        ...
        header_utils = Column(
            children=[label, Row(children=[self._text_input, error_message])]
        )

        plot_grid = gridplot(
            children=[
                [self._s1.figure, self._s2.figure, self._s3.figure],
