...
def initialize_app(server_information: ServerInformation) -> ServerInformation:
    ...
    def create_application(doc: Document) -> None:
        ...
        doc.template_variables["data_server_host"] = server_information.get_application_information(
            "flask_server_host")
        doc.template_variables["data_server_port"] = server_information.get_application_information(
            "flask_server_port")
    ...
    async def log_messages():
        ...
        if max_number_of_elements:
            ...
            response = [(index,) + tuple(r) for index, r in
                        zip(response.index.to_numpy(), response.to_numpy())]
